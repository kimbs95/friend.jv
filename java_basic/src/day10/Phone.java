...
public class Phone {
    ...
    //             선언하지 않으면 자동으로 선언됨.
    // 생성자는 기본값이 무조건 void 라서 앞에 타입을 안씁니다.
    Phone() {
    ...
    void showSpec() {
        ...
        if (!on) {
            System.out.println(model + "의 전원을 먼저켜세요!");
            return;
        }
    ...
    boolean sendMessage(Phone targetPhone, String message) {
        ...
        if (targetPhone.receiveMessages.length > targetPhone.messageCount) {  // 가득차지 않았다면
            // 메시지를 상대방 메시지함에 저장!
            targetPhone.receiveMessages[targetPhone.messageCount++] = String.format("%s (%s)", message, model);
            return true;
